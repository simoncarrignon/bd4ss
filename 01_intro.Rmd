...
```{r}
 str(mpg)
```
You can see from the output that this is a `data.frame`:  a rectangular structure with column and lines (`?data.frame` if you want to learn more `data.frame`). In this example all columns have the same type but they may have different types.
