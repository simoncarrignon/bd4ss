...
* To get help about anything use `?`: `?seq` will show you the manual to  use the function `seq` (which generate sequences of numbers)
...
```{r}
 str(mpg)
```
You cna see from the output that this is a `data.frame`:  a rectangular structure with column and lines (`?data.frame` if you want to learn more about them
