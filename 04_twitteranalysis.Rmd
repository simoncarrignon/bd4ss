...
Different datasets exist that give for huge list of words the associated emotion. the "tidytext" library allows to use some of those dataset through the call of the function `get_sentiments`. You can for example load the [NRC word-emotion association lexicon](https://www.saifmohammad.com/WebPages/NRC-Emotion-Lexicon.htm) by doing: 
```{r}
